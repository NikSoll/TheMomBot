...
class MailerDatabase:
    def __init__(self, creds_file="creds5.json"):
    ...
    def connect(self):
        try:
            ...
            if MAILER_SETTINGS.get('sheet_url'):
        ...
        try:
            ...
            for i, record in enumerate(records, start=2):
                if str(record.get('ID пользователя', '')) == str(user_id):
                    self.subscribers_sheet.update_cell(i, 5, "Нет")  # Активен
                    return True
        except:
            pass
        return False
    ...
    def get_group_subscribers(self, group_id):
        # В реальном проекте здесь фильтрация по группам
